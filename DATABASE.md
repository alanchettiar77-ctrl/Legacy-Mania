...
- **banners** *(new in `003`)* — id (PK), title, description, image_url, category_id (FK→categories, cascade), display_order, is_active, timestamps, deleted_at (soft delete). Public RLS: active + not deleted only; no anon write policies.
