...
- **banners** *(replaced in `010` — migration 003 version is dead schema)* — id (PK), title, subtitle, cta_text, cta_url, category_id (FK→categories, `SET NULL`), desktop_image_url, mobile_image_url, alt_text, aria_label, image_title, overlay_enabled, overlay_opacity (0–1, default 0.4), banner_type (`image`|`video`|`promotional`|`seasonal`, default `image`), video_url (unused unless video type), display_order, is_active, start_date/end_date (scheduling window, CHECK end > start), seo_meta_title/seo_meta_description/seo_keywords (future-ready SEO fields), og_title/og_description/og_image_url, canonical_url, schema_type, created_by/updated_by (FK→profiles), timestamps, deleted_at (soft delete). Public RLS: live banners only (active + not deleted + within schedule window); admin-read via `is_admin()`; no anon write policies — all writes via service-role API routes.
