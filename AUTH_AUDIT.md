...
**Status: fixed 2026-07-21.** All ~13 browser-client call sites were audited and every session-dependent one migrated to a server route:
- **New backend surfaces:** `address-repository`/`address-service` + `/api/account/addresses` (+`/[id]`) for full address CRUD; `/api/account/password` for password changes; `/api/auth/logout`; `product-repository`/`product-service` + `/api/admin/products` (+`/[id]`); `category-repository` extended (`insertCategory`) + `category-service` + `/api/admin/categories` (+`/[id]`); `settings-repository`/`settings-service` + `/api/admin/settings`; a new `upi` MediaService namespace (reuses the `banners` bucket, same pattern as `branding`) for the QR upload.
- **Reused existing routes** (no new backend needed): `orders-table-client.tsx`'s quick-update was bypassing the already-correct `PATCH /api/admin/orders/[id]/status` and `PATCH /api/admin/payments/[id]/verify` — rewired to call them instead of writing to `orders`/`payments` directly (this was also a real service-layer-bypass bug, independent of the cookie issue: it skipped `OrderService`'s guarded status transitions and `PaymentService`'s verify audit trail).
- **Password-reset flow required a server-side PKCE exchange:** the recovery link now redirects to a new `/auth/confirm` route (`exchangeCodeForSession` via the server client, setting the session cookie httpOnly) instead of directly to `/reset-password`, which previously exchanged the code in the browser client and would otherwise have overwritten the httpOnly cookie non-httpOnly the moment it touched auth state. `reset-password/page.tsx` now POSTs to `/api/account/password` instead of calling `updateUser()` client-side.
- **Left alone, confirmed safe:** `forgot-password/page.tsx`'s `resetPasswordForEmail()` call (browser client, but doesn't touch the main session cookie — only the separate PKCE code-verifier cookie, and no session exists yet at that point); `wishlist/page.tsx` and `checkout-client.tsx`'s settings read (both anonymous/public data reads, not session-dependent).
- **Cookie flags:** `cookieOptions: { httpOnly: true, secure: process.env.NODE_ENV === "production" }` added to both `server.ts` and `middleware.ts` (the two places that write the session cookie server-side). `client.ts` (browser client) is untouched — it structurally cannot set `httpOnly` (JS-set cookies never can), and no longer needs to manage session state at all.
- Verified via `npm run build` (production build, all new routes compile) + full test suite (302 tests) + `tsc --noEmit` (clean).
