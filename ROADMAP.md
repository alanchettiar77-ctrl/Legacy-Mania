...
| 2 — Banners (CMS Phase 1) | Full banner feature (admin CRUD/reorder, image upload, scheduling, overlay, SEO fields, homepage carousel); product catalog ordering fixed to default to `display_order` ASC (migration 011, backfilled per category, live sort options: Display Order/Featured/Newest/Oldest/Price↑↓/A-Z/Z-A), admin Display Order field with drag-reorder | **✅ Complete (2026-07-26)** — note: the homepage notification engine (2026-07-19, migration 007) already covers the scrolling text bar; Phase 2 is the image banner carousel + product display order |
