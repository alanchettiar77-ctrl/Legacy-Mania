...
| 4b — Testimonials CMS | Admin-manageable customer testimonials (currently 4 hardcoded fake reviews in `src/components/home/testimonials.tsx` — name/location/avatar/text/rating/series, no database, no admin UI). Requested by user 2026-07-27, not previously scoped in any prior plan. Follows the established `banners`/`hero_tiles` Repository → Service → API → Admin UI pattern. | Not started |
